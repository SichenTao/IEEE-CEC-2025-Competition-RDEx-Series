...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run the RDEx-CSOP algorithm.")
...
def build_binary(root: Path, cxx: str) -> Path:
    ...
    binary = build / "rdex_csop"
    subprocess.run(
        [cxx, "-O3", "-std=c++11", "RDEx.cpp", "-o", str(binary)],
        cwd=str(root),
        check=True,
    )
    return binary
...
def main() -> int:
    ...
    shutil.copytree(root / "inputData", args.out_dir / "inputData", dirs_exist_ok=True)
